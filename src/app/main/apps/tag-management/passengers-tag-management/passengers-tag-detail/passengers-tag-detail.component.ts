...
import {MerchantsTagManagementEntity, TagManagementService} from '../../../../../services/tagManagementService/tag-management.service';
...
@Component({
  selector: 'app-passengers-tag-detail',
  templateUrl: './passengers-tag-detail.component.html',
  styleUrls: ['./passengers-tag-detail.component.scss']
})
export class PassengersTagDetailComponent implements OnInit{
  ...
  getTagManagementDetail() {
    this.passengersTagService.getDetailById(this.TagDetailId).subscribe(res => {
      if (res['enabled'] === true) {
        res['enabled'] = '正常';
      } else {
        res['enabled'] = '冻结';
  ...
  onSave() {
    ...
    this.passengersTagService.createPassengerTagList(this.profileForm.value).subscribe(
        () => {
          this.router.navigate(['/apps/PassengersTagManagement']).then();
          this.snackBar.open('新增标签成功', '✖');
        }, () => {
  ...
  onUpdate(){
    ...
    this.profileForm.value['enabled'] = true;
    this.saveButtonStatus = true;
    this.passengersTagService.updatePassengerTagList(this.profileForm.value).subscribe(
        () => {
          this.router.navigate(['/apps/PassengersTagManagement']).then();
          this.snackBar.open('编辑标签成功', '✖');
        }, () => {
          this.saveButtonStatus = false;
        }, () => {
