...
@Component({
    selector: 'app-applet-mask-copy',
    templateUrl: './applet-mask-copy.component.html',
    styleUrls: ['./applet-mask-copy.component.scss'],
    animations: fuseAnimations
})
// 新增
export class AppletMaskCopyComponent implements OnInit, OnDestroy {
    ...
    constructor(
        ...
        this.options2 = new FormBuilder().group({
            ...
            popupStatus: new FormControl({
                value: this.translate.instant('AppletMask.addDetail.invalid') // 无效
                , disabled: true
            }, [Validators.required]),
        });
    }
                        ...
                        this.appletMaskServiceService.getPopupConfigurations(this.POPUPID).pipe(takeUntil(this._unsubscribeAll)).subscribe(
                            res_s => {
                                ...
                                if (this.options2.controls.popupStatus.value === this.translate.instant('AppletMask.addDetail.invalid')
                                    || this.options2.controls.popupStatus.value === this.translate.instant('AppletMask.addDetail.prohibit')) {
    ...
    popSave(p) {
        ...
        if (p === 'add') {
            ...
            // 上架
            // this.appletMaskServiceService.setActivityPopup(this.POPUPID).pipe(takeUntil(this._unsubscribeAll)).subscribe(res => {
            //     this.btuDis = false;
            //     this.frozenY = 'LowerShelf';
            ...
            // 下架
            this.appletMaskServiceService.putPopupConfigurations(this.POPUPID).pipe(takeUntil(this._unsubscribeAll)).subscribe(res => {
    ...
    HandleCoupon(activitySource) {
        ...
        /*  else if (activitySource['activityType'] ===  AppletMaskParameter.AC_INTEGRAL) {
                    // 积分活动
                }else if (activitySource['activityType'] ===  AppletMaskParameter.AC_GROUPBUY) {
