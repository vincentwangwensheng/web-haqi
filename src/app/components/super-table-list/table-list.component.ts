...
export class TableListComponent implements OnInit, OnDestroy, AfterViewInit {
    ...
    @Input()
    directShow = false; // 直接显示字段不进行管道处理

    @Input()
    page: any = {page: 0, size: 20, sort: '', count: 0};
    initSort = ''; // 初始化时的排序
    // 上次操作的分页
    lastPage = 0;
    ...
    ngOnInit() {
        ...
        // 是否有查询权限
        this.permissionsService.hasPermission(this.searchPermissions).then(res => {
            this.canSearch = res;
    ...
    onResponseReturn() {
        const onResponse = this.notify.onResponse.subscribe(() => {
            if (!this.canSearch) {
                this.snackBar.open(this.translate.instant('tableList.cannotSearch'), '！');
                return;
            }
    ...
    // 显示选择
    showSelect() {
        this.selectLeft = window.innerWidth - this.dataTable.element.offsetWidth - 24 + 'px';
    ...
    // 上传列表图片
    onUploadProgress(event, row, columnName) {
        if (event.data) {
            row[columnName] = event.data;
        }
    }
...
export interface Column {
    ...
    type?: 'input' | 'select' | 'date' | 'image' | ''; // 类型
    value?: any; // 每列查询的值
    options?: any[]; // select下拉选
    alignClass?: 'align-left' | 'align-right' | 'align-center'; // 默认样式是居中了
    transform?: []; // 传如对应的转化参数 默认为keyValueTransform 管道
    transformType?: 'map' | 'replace' | 'date' | 'htmlToText';  // 对应 arrayFieldMap  | transformInArray 管道 | 转化日期管道 | html转text管道
}
